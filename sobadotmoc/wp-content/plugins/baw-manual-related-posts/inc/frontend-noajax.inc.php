		...
		if ( ! empty( $ids ) && is_array( $ids ) && isset( $ids[0] ) && $ids[0] != 0 ) {
			...
			foreach( $ids as $id ) {
				...
				if( $bawmrp_options['in_content_mode']=='list' ) {
					...
					$list[] = '<li style="' . esc_attr( $style ) . '" class="' . $class . '">ssssssssssssssssssssssssss<a href="' . esc_url( apply_filters( 'the_permalink', get_permalink( $id ) ) ) . '">' . $thumb . '<br />' . apply_filters( 'the_title', get_the_title( $id ) ) . '</a>' . the_excerpt( $id ) .'</li>';
				}
